...
def process(from_index, to_index, num, old_length):
    length = (to_index-from_index)
    img_ = Image.new('RGB', (length * 64,size_y * 64))
    if from_index == 0:
        for i in tqdm(range(length * size_y)):
            temp_ = Image.open("dataset/" + str(out_better[i // length][i % length]) + ".jpg")
            img_.paste(temp_, (i % length * 64, i // length * 64))
    else:
        for i in range((to_index-from_index) * size_y):
            temp_ = Image.open("dataset/" + str(out_better[i // length][i % length + old_length * num]) + ".jpg")
...
if __name__ == "__main__":
    ...
    for i in range(thread_count - 1):
        procceses.append(multiprocessing.Process(target=process, args=(
                    ...
                    size_x // thread_count * (i + 1), i, size_x // thread_count)))
    procceses.append(multiprocessing.Process(target=process, args=(
                    size_x // thread_count * (thread_count - 1), size_x, thread_count-1, size_x // thread_count)))
